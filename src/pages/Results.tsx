...
const Results = () => {
  ...
  const getSentimentIcon = (sentiment: string) => {
    switch (sentiment) {
      case 'positive':
        return <ThumbsUp className="h-8 w-8 text-green-400" />;
      case 'negative':
        return <ThumbsDown className="h-8 w-8 text-red-400" />;
      default:
  ...
  const getSentimentGradient = (sentiment: string) => {
    switch (sentiment) {
      ...
      case 'negative':
        return 'from-red-400/20 to-rose-400/20';
      default:
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center relative overflow-hidden">
        ...
        <Card className="w-96 shadow-2xl bg-white/10 backdrop-blur-xl border-white/20 relative">
          <CardContent className="flex flex-col items-center py-16">
            ...
            <div className="text-center space-y-4">
              ...
              <p className="text-gray-300">Analyzing sentiment patterns...</p>
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-300 text-sm">Analysis Complete</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="relative container mx-auto px-4 py-12">
        <div className="max-w-6xl mx-auto">
          ...
          <Button 
            variant="ghost" 
            onClick={() => navigate("/")}
            className="mb-8 text-white hover:bg-white/10 border border-white/20 backdrop-blur-sm"
          >
          ...
          <Card className={`mb-12 shadow-2xl border-0 bg-gradient-to-br ${getSentimentGradient(result.sentiment)} backdrop-blur-xl relative overflow-hidden transition-all duration-1000 ${animateResults ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            <CardContent className="text-center relative">
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 mb-8 border border-white/20">
                <p className="text-lg text-gray-200 mb-6 font-medium flex items-center justify-center gap-2">
                ...
                <Progress value={result.confidence} className="h-4 bg-white/20" />
              </div>
              <p className="text-gray-200 text-xl leading-relaxed bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/20">
                {result.explanation}
              </p>
            </CardContent>
          </Card>
              ...
              <CardContent className="space-y-6">
                <div>
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span className="font-bold text-green-300 text-lg">{Math.round(result.positiveScore)}%</span>
                  </div>
                  <Progress value={result.positiveScore} className="h-3 bg-white/20" />
                </div>
                <div>
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span className="font-bold text-red-300 text-lg">{Math.round(result.negativeScore)}%</span>
                  </div>
                  <Progress value={result.negativeScore} className="h-3 bg-white/20" />
                </div>
                <div>
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span className="font-bold text-yellow-300 text-lg">{Math.round(result.neutralScore)}%</span>
                  </div>
                  <Progress value={result.neutralScore} className="h-3 bg-white/20" />
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-2xl border-0 bg-white/10 backdrop-blur-xl border-white/20">
              ...
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 gap-4">
                  ...
                  <div className="flex justify-between items-center p-3 bg-white/5 rounded-lg">
                    <span className="text-gray-300">Processed At:</span>
          ...
          {/* Key Words Analysis */}
          <Card className={`mb-12 shadow-2xl border-0 bg-white/10 backdrop-blur-xl border-white/20 transition-all duration-1000 delay-500 ${animateResults ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <CardHeader>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {result.keyWords.positive.map((word, index) => (
                      <Badge key={index} variant="outline" className="bg-green-500/20 text-green-300 border-green-500/50 px-3 py-1 hover:bg-green-500/30 transition-all duration-200">
                        {word}
                      </Badge>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {result.keyWords.negative.map((word, index) => (
                      <Badge key={index} variant="outline" className="bg-red-500/20 text-red-300 border-red-500/50 px-3 py-1 hover:bg-red-500/30 transition-all duration-200">
                        {word}
                      </Badge>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {result.keyWords.neutral.map((word, index) => (
                      <Badge key={index} variant="outline" className="bg-yellow-500/20 text-yellow-300 border-yellow-500/50 px-3 py-1 hover:bg-yellow-500/30 transition-all duration-200">
                        {word}
                      </Badge>
                    ))}
                  </div>
          ...
          {/* Original Review */}
          <Card className={`shadow-2xl border-0 bg-white/10 backdrop-blur-xl border-white/20 transition-all duration-1000 delay-700 ${animateResults ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <CardHeader>
            ...
            <CardContent>
              <div className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border-l-4 border-purple-500 border border-white/20">
